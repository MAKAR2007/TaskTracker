...
export default function Board() {
  ...
  }, [selected, taskStage, allUsers, data, allProjects, setSelected]);
  ...
  /*
  function createUniqueImages({ boardData }) {
    let images = [];
    for (let ii = 0; ii < boardData.length; ii++) {
      for (let jj = 0; jj < boardData[ii].tasks.length; jj++) {
        if (boardData[ii].tasks[jj].user?.image) {
          images.push(boardData[ii].tasks[jj].user.image);
        }
      }
    }
    const uImages = Array.from(new Set(images));
    return uImages;
  }
  let uniqueImages = createUniqueImages({ boardData });
*/
  return (
    <div className=" bg-bgmain-100 mx-auto h-screen max-w-5xl p-10 2xl:ml-72 2xl:max-w-full">
      {/* Board header */}
      <div className="flex flex-col items-start lg:flex-row  lg:items-center ">
        <div className="">
          ...
          >
            <div className="relative mt-1">
              <Listbox.Button className="focus-visible:border-accent-500 relative w-full cursor-default  rounded-lg py-2  pr-10    text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                <span className="text-accent-600 mx-4 block truncate text-4xl font-bold">
            ...
            {selected && selected.managerId ? (
              <>
                <Image
                  ...
                  width="36"
                  height="36"
                  className="mx-4  inline-block rounded-full "
                  alt="some image 1"
                />
        ...
        <ul className="flex flex-shrink-0 space-x-3">
